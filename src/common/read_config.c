...
static void _push_to_hashtbls(char *alias, char *hostname, char *address,
                              ...
                              slurm_addr_t *addr, bool initialized,
                              bool dynamic, bool cloud);
...
s_p_options_t slurm_conf_options[] = {
    ...
    {"PartitionName", S_P_ARRAY, _parse_partitionname, _destroy_partitionname},
...
static int _defunct_option(void **dest, slurm_parser_enum_t type,
                           ...
                           char **leftover) {
  error_in_daemon(
      "The option \"%s\" is defunct, please remove it from slurm.conf.", key);
  return 0;
}
...
static int _parse_nodename(void **dest, slurm_parser_enum_t type,
  ...
  static s_p_options_t _nodename_options[] = {
      ...
      {"RestrictedMemPerGPU", S_P_UINT16},
      {"Sockets", S_P_UINT16},
  ...
  if (xstrcasecmp(value, "DEFAULT") == 0) {
    ...
    fatal("'%s' is a reserved word disallowed for use with NodeName", value);
  } else {
    bool no_cpus = false;
    ...
    if (s_p_get_string(&cpu_bind, "CpuBind", tbl) ||
      ...
      if (xlate_cpu_bind_str(cpu_bind, &n->cpu_bind) != SLURM_SUCCESS) {
    ...
    if (!s_p_get_uint16(&n->core_spec_cnt, "CoreSpecCount", tbl))
      s_p_get_uint16(&n->core_spec_cnt, "CoreSpecCount", dflt);
    ...
    if (!s_p_get_uint64(&n->mem_spec_limit, "MemSpecLimit", tbl))
      s_p_get_uint64(&n->mem_spec_limit, "MemSpecLimit", dflt);
    ...
    if (!s_p_get_uint16(&n->res_cores_per_gpu, "RestrictedCoresPerGPU", tbl))
      s_p_get_uint16(&n->res_cores_per_gpu, "RestrictedCoresPerGPU", dflt);

    if (!s_p_get_uint16(&n->res_cores_per_gpu, "RestrictedMemPerGPU", tbl))
      s_p_get_uint16(&n->res_cores_per_gpu, "RestrictedMemPerGPU", dflt);
    ...
    if (!s_p_get_uint16(&sockets_per_board, "SocketsPerBoard", tbl) &&
        !s_p_get_uint16(&sockets_per_board, "SocketsPerBoard", dflt)) {
      sockets_per_board = 1;
    ...
    if (!no_sockets_per_board && !no_sockets) {
      error_in_daemon("NodeNames=%s Sockets=# and SocketsPerBoard=# is invalid "
    ...
    if (no_sockets) {
      ...
      } else if (!no_cpus && (n->cpus / (n->cores * n->threads)) &&
                 !(n->cpus % (n->cores * n->threads))) {
        /* infer missing Sockets= based on cpus */
        n->tot_sockets = n->cpus / (n->cores * n->threads);
        debug("NodeNames=%s setting Sockets=%d based on "
              "CPUs(%d)/(CoresPerSocket(%d)/ThreadsPerCore(%d))",
              n->nodenames, n->tot_sockets, n->cpus, n->cores, n->threads);
      } else {
        /* default to one socket per board */
        n->tot_sockets = n->boards;
        debug("NodeNames=%s setting Sockets=Boards(%d)", n->nodenames,
              n->boards);
      }
    }
    ...
    if (n->tot_sockets < n->boards) {
      error_in_daemon(
          "NodeNames=%s Sockets(%d) < Boards(%d) resetting Boards=1",
          n->nodenames, n->tot_sockets, n->boards);
      n->boards = 1;
    }

    /* Node boards are factored into sockets */
    if ((n->cpus != n->tot_sockets) && (n->cpus != n->tot_sockets * n->cores) &&
        (n->cpus != n->tot_sockets * n->cores * n->threads)) {
      error_in_daemon(
          "NodeNames=%s CPUs=%d match no Sockets, Sockets*CoresPerSocket or "
          "Sockets*CoresPerSocket*ThreadsPerCore. Resetting CPUs.",
          n->nodenames, n->cpus);
      n->cpus = n->tot_sockets * n->cores * n->threads;
    }
    ...
    if (n->cpu_spec_list) {
      ...
      if (bit_unfmt(cpu_spec_bitmap, n->cpu_spec_list)) {
        error_in_daemon("NodeNames=%s CpuSpecList=%s - unable to convert it to "
                        "bitmap of size CPUs=%d. Ignoring CpuSpecList.",
    ...
    if ((n->core_spec_cnt > 0) && n->cpu_spec_list) {
      error_in_daemon("NodeNames=%s CoreSpecCount=%u is invalid with "
                      "CPUSpecList, reset to 0",
                      n->nodenames, n->core_spec_cnt);
      n->core_spec_cnt = 0;
    }

    if (n->mem_spec_limit >= n->real_memory) {
      error_in_daemon("NodeNames=%s MemSpecLimit=%" PRIu64
                      " is invalid, reset to 0",
 ...
 *
 */
static uint16_t *_parse_srun_ports(const char *str) {
  char *min;
  char *max;
  char *dash;
  char *p;
...
extern int job_defaults_list(char *in_str, list_t **out_list) {
  ...
  *out_list = NULL;
  ...
  while (tok) {
    ...
    if (!end_ptr || (end_ptr[0] != '\0') || (value < 0) ||
        (value == LLONG_MAX)) {
      rc = EINVAL;
      break;
    }
...
extern char *job_defaults_str(list_t *in_list) {
  ...
  iter = list_iterator_create(in_list);
  while ((in_default = list_next(iter))) {
    xstrfmtcat(out_str, "%s%s=%" PRIu64, sep, _job_def_name(in_default->type),
               in_default->value);
    sep = ",";
  }
  list_iterator_destroy(iter);
...
static int _parse_partitionname(void **dest, slurm_parser_enum_t type,
  ...
  if (xstrcasecmp(value, "DEFAULT") == 0) {
    ...
    if (!s_p_get_string(&p->allow_alloc_nodes, "AllocNodes", tbl)) {
      s_p_get_string(&p->allow_alloc_nodes, "AllocNodes", dflt);
    ...
    if (s_p_get_string(&cpu_bind, "CpuBind", tbl) ||
        s_p_get_string(&cpu_bind, "CpuBind", dflt)) {
      if (xlate_cpu_bind_str(cpu_bind, &p->cpu_bind) != SLURM_SUCCESS) {
        error("Partition=%s CpuBind=\'%s\' is invalid, ignored", p->name,
              cpu_bind);
        p->cpu_bind = 0;
      }
      xfree(cpu_bind);
    ...
    if (!s_p_get_string(&p->billing_weights_str, "TRESBillingWeights", tbl))
      s_p_get_string(&p->billing_weights_str, "TRESBillingWeights", dflt);

    if (!s_p_get_boolean(&p->default_flag, "Default", tbl))
      s_p_get_boolean(&p->default_flag, "Default", dflt);

    if (!s_p_get_uint32(&p->max_cpus_per_node, "MaxCPUsPerNode", tbl))
      s_p_get_uint32(&p->max_cpus_per_node, "MaxCPUsPerNode", dflt);

    if (!s_p_get_uint32(&p->max_cpus_per_socket, "MaxCPUsPerSocket", tbl))
      s_p_get_uint32(&p->max_cpus_per_socket, "MaxCPUsPerSocket", dflt);
    ...
    if (!s_p_get_uint64(&p->def_mem_per_cpu, "DefMemPerNode", tbl) &&
        !s_p_get_uint64(&p->def_mem_per_cpu, "DefMemPerNode", dflt)) {
      if (s_p_get_uint64(&p->def_mem_per_cpu, "DefMemPerCPU", tbl) ||
          s_p_get_uint64(&p->def_mem_per_cpu, "DefMemPerCPU", dflt)) {
        p->def_mem_per_cpu |= MEM_PER_CPU;
      }
    } else if (s_p_get_uint64(&tmp_64, "DefMemPerCPU", tbl) ||
               s_p_get_uint64(&tmp_64, "DefMemPerCPU", dflt)) {
      error("DefMemPerCPU ignored, since it's mutually exclusive with "
            "DefMemPerNode");
    }

    if (!s_p_get_uint64(&p->max_mem_per_cpu, "MaxMemPerNode", tbl) &&
        !s_p_get_uint64(&p->max_mem_per_cpu, "MaxMemPerNode", dflt)) {
      if (s_p_get_uint64(&p->max_mem_per_cpu, "MaxMemPerCPU", tbl) ||
          s_p_get_uint64(&p->max_mem_per_cpu, "MaxMemPerCPU", dflt)) {
        p->max_mem_per_cpu |= MEM_PER_CPU;
      }
    } else if (s_p_get_uint64(&tmp_64, "MaxMemPerCPU", tbl) ||
               s_p_get_uint64(&tmp_64, "MaxMemPerCPU", dflt)) {
      error("MaxMemPerCPU ignored, since it's mutually exclusive with "
            "MaxMemPerNode");
    }

    s_p_get_boolean((bool *)&p->disable_root_jobs, "DisableRootJobs", tbl);

    s_p_get_boolean((bool *)&p->exclusive_user, "ExclusiveUser", tbl);

    s_p_get_boolean((bool *)&p->exclusive_topo, "ExclusiveTopo", tbl);
    ...
    if (s_p_get_string(&tmp, "OverTimeLimit", tbl) ||
      ...
      if (!strcasecmp(tmp, "INFINITE") || !strcasecmp(tmp, "UNLIMITED")) {
        p->over_time_limit = INFINITE16;
      } else {
        int i = strtol(tmp, (char **)NULL, 10);
        if (i < 0)
          error("Ignoring bad OverTimeLimit value: %s", tmp);
        else if (i > 0xfffe)
          p->over_time_limit = INFINITE16;
        else
          p->over_time_limit = i;
    ...
    if (s_p_get_string(&tmp, "PreemptMode", tbl) ||
      ...
      if (p->preempt_mode & PREEMPT_MODE_GANG) {
        error_in_daemon("PreemptMode=GANG is a cluster-wide option and cannot "
                        "be set at partition level, option ignored.");
    ...
    if (!s_p_get_uint16(&p->priority_job_factor, "PriorityJobFactor", tbl))
      s_p_get_uint16(&p->priority_job_factor, "PriorityJobFactor", dflt);
    ...
    if (s_p_get_string(&tmp, "SelectTypeParameters", tbl)) {
      if (xstrncasecmp(tmp, "CR_Core_Memory", 14) == 0)
        ...
        error("Bad value for SelectTypeParameters: %s", tmp);
        _destroy_partitionname(p);
        s_p_hashtbl_destroy(tbl);
        xfree(tmp);
        return -1;
    ...
    if (s_p_get_string(&tmp, "OverSubscribe", tbl) ||
      ...
      if (xstrcasecmp(tmp, "NO") == 0)
        ...
        if (i <= 1) {
          error("Ignoring bad OverSubscribe value: %s", tmp);
        ...
        error("Bad value \"%s\" for OverSubscribe", tmp);
        _destroy_partitionname(p);
        s_p_hashtbl_destroy(tbl);
        xfree(tmp);
        return -1;
    ...
    if (s_p_get_string(&tmp, "SuspendTime", tbl)) {
      if (!xstrcasecmp(tmp, "INFINITE") || !xstrcasecmp(tmp, "-1")) {
    ...
    if (!s_p_get_uint16(&p->suspend_timeout, "SuspendTimeout", tbl))
      s_p_get_uint16(&p->suspend_timeout, "SuspendTimeout", dflt);
...
static int _load_slurmctld_host(slurm_conf_t *conf) {
  ...
  if (s_p_get_array((void ***)&ptr, &count, "SlurmctldHost", conf_hashtbl)) {
    ...
    if (!s_p_get_string(&conf->control_machine[0], "ControlMachine",
                        conf_hashtbl)) {
      /*
    ...
    if (!s_p_get_string(&conf->control_addr[0], "ControlAddr", conf_hashtbl) &&
        conf->control_machine[0] && strchr(conf->control_machine[0], ',')) {
      error("ControlMachine has multiple host names, so ControlAddr must be "
            "specified.");
      goto error;
    }
  ...
  for (i = 0; i < conf->control_cnt; i++) {
    if (!conf->control_addr[i]) {
      conf->control_addr[i] = xstrdup(conf->control_machine[i]);
    }
    if (!xstrcasecmp("localhost", conf->control_machine[i])) {
      xfree(conf->control_machine[i]);
      conf->control_machine[i] = xmalloc(HOST_NAME_MAX);
      if (gethostname_short(conf->control_machine[i], HOST_NAME_MAX)) {
        error("getnodename: %m");
        goto error;
      }
    }
    for (j = 0; j < i; j++) {
      if (!xstrcmp(conf->control_machine[i], conf->control_machine[j])) {
        error("Duplicate SlurmctldHost records: %s", conf->control_machine[i]);
        goto error;
      }
    }
  }
...
static int _parse_slurmctld_host(void **dest, slurm_parser_enum_t type,
  ...
  static s_p_options_t _slurmctld_host_options[] = {{NULL}};
...
int slurm_conf_partition_array(slurm_conf_partition_t **ptr_array[]) {
  ...
  if (s_p_get_array((void ***)&ptr, &count, "PartitionName", conf_hashtbl)) {
    *ptr_array = ptr;
    return count;
  } else {
    *ptr_array = NULL;
...
static int _parse_downnodes(void **dest, slurm_parser_enum_t type,
  ...
  static s_p_options_t _downnodes_options[] = {
      {"Reason", S_P_STRING}, {"State", S_P_STRING}, {NULL}};
...
static int _parse_nodeset(void **dest, slurm_parser_enum_t type,
  ...
  static s_p_options_t _nodeset_options[] = {
      {"Feature", S_P_STRING}, {"Nodes", S_P_STRING}, {NULL}};
...
static void _push_to_hashtbls(char *alias, char *hostname, char *address,
  ...
  while (p) {
    if (xstrcmp(p->hostname, hostname) == 0) {
      error("Duplicated NodeHostName %s in the config file", hostname);
      return;
    }
    p = p->next_hostname;
  }
  ...
  while (p) {
    if (xstrcmp(p->alias, alias) == 0) {
      fatal("Duplicated NodeName %s in the config file", p->alias);
    }
    p = p->next_alias;
  }
...
static int _check_callback(char *alias, char *hostname, char *address,
  ...
  if (!cloud_dns &&
      ((state_val & NODE_STATE_CLOUD) || (state_val & NODE_STATE_FUTURE)))
    dynamic_addr = true;

  if (!running_in_slurmctld() && (state_val & NODE_STATE_CLOUD))
    cloud_node = true;

  _push_to_hashtbls(alias, hostname, address, bcast_address, port, NULL, false,
                    dynamic_addr, cloud_node);
  return SLURM_SUCCESS;
}
...
extern char *slurm_conf_get_aliased_nodename(void) {
  ...
  for (addr_ptr = addrs; addr_ptr; addr_ptr = addr_ptr->ai_next) {
    if (addr_ptr->ai_canonname) {
      nodename = slurm_conf_get_nodename(addr_ptr->ai_canonname);
    } else {
      slurm_addr_t addr = {0};
  ...
  while (p_curr) {
    if (p_curr == p) {
      if (p_prev)
        ...
        host_to_node_hashtbl[hostname_idx] = p_curr->next_hostname;
      break;
    }
    p_prev = p_curr;
...
extern void slurm_reset_alias(char *node_name, char *node_addr,
  ...
  if (!p) {
    _push_to_hashtbls(node_name, node_hostname, node_addr, NULL, 0, NULL, false,
                      false, false);
  }
  slurm_conf_unlock();
...
extern int slurm_conf_get_addr(const char *node_name, slurm_addr_t *address,
  ...
  if (p->bcast_address && (flags & USE_BCAST_NETWORK)) {
    if (!p->bcast_addr_initialized) {
      slurm_set_addr(&p->bcast_addr, p->port, p->bcast_address);
...
extern slurm_conf_node_t *
slurm_conf_parse_nodeline(const char *nodeline, s_p_hashtbl_t **out_hashtbl) {
  int count = 0;
  slurm_conf_node_t **ptr_array;
  s_p_hashtbl_t *node_hashtbl = NULL;
  char *leftover = NULL;
  s_p_options_t node_options[] = {
      {"NodeName", S_P_ARRAY, _parse_nodename, _destroy_nodename}, {NULL}};
  ...
  if (!s_p_get_array((void ***)&ptr_array, &count, "NodeName", node_hashtbl)) {
    s_p_hashtbl_destroy(node_hashtbl);
  ...
  if (count != 1) {
    s_p_hashtbl_destroy(node_hashtbl);
    error("Failed to find one NodeName in nodeline: '%s'", nodeline);
    return NULL;
  }
...
static int _init_slurm_conf(const char *file_name) {
  ...
  if (running_in_slurmd())
    conf_buf = s_p_pack_hashtbl(conf_hashtbl, slurm_conf_stepd_options,
...
/* caller must lock conf_lock */
static void _destroy_slurm_conf(void) {
...
static int _establish_config_source(char **config_file, bool *memfd) {
   ...
   */
  if (*config_file) {
    debug2("%s: using config_file=%s (provided)", __func__, *config_file);
    return SLURM_SUCCESS;
  }
  if ((*config_file = xstrdup(getenv("SLURM_CONF")))) {
    debug("%s: using config_file=%s (environment)", __func__, *config_file);
    return SLURM_SUCCESS;
  }
  ...
  if (!stat(default_slurm_config_file, &stat_buf)) {
    *config_file = xstrdup(default_slurm_config_file);
    debug2("%s: using config_file=%s (default)", __func__, *config_file);
    return SLURM_SUCCESS;
  }
  ...
  if (!stat("/run/slurm/conf/slurm.conf", &stat_buf)) {
    *config_file = xstrdup("/run/slurm/conf/slurm.conf");
    debug2("%s: using config_file=%s (cached)", __func__, *config_file);
    return SLURM_SUCCESS;
  }
   ...
   */
  if (!(config = fetch_config(NULL, CONFIG_REQUEST_SLURM_CONF, 0, NULL)) ||
      !config->config_files) {
   ...
   */
  if (!(config_tmp =
            list_find_first(config_files, find_conf_by_name, "slurm.conf"))) {
    error("%s: No slurm.conf found in configuration files received.", __func__);
    return SLURM_ERROR;
  }
...
extern void read_conf_recv_stepd(int fd) {
  ...
  conf_hashtbl = s_p_unpack_hashtbl_full(conf_buf, slurm_conf_stepd_options);
rwfail:
  FREE_NULL_BUFFER(conf_buf);
}
...
extern void slurm_conf_init_stepd(void) {
  ...
  } else if ((parse_rlimits(slurm_conf.propagate_rlimits, PROPAGATE_RLIMITS)) <
             0) {
    error("Bad PropagateResourceLimits: %s", slurm_conf.propagate_rlimits);
    return;
  }
...
extern void slurm_conf_mutex_init(void) { slurm_mutex_init(&conf_lock); }

extern void slurm_conf_install_fork_handlers(void) {
  int err;
...
extern void slurm_conf_unlock(void) { slurm_mutex_unlock(&conf_lock); }
...
static void _normalize_debug_level(uint16_t *level) {
  if (*level > LOG_LEVEL_END) {
    error("Normalizing debug level from %u to %d", *level, (LOG_LEVEL_END - 1));
...
static uint16_t _health_node_state(char *state_str) {
  ...
  while (token) {
    if (!xstrcasecmp(token, "ANY")) {
      ...
      error("Invalid HealthCheckNodeState value %s ignored", token);
    }
...
static int _validate_accounting_storage_enforce(char *acct_enforce_str,
  ...
  while (tok) {
    if (!xstrcasecmp(tok, "1") || !xstrcasecmp(tok, "associations")) {
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_ASSOCS;
    } else if (!xstrcasecmp(tok, "2") || !xstrcasecmp(tok, "limits")) {
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_ASSOCS;
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_LIMITS;
    } else if (!xstrcasecmp(tok, "safe")) {
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_ASSOCS;
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_LIMITS;
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_SAFE;
    } else if (!xstrcasecmp(tok, "wckeys")) {
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_ASSOCS;
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_WCKEYS;
      conf->conf_flags |= CONF_FLAG_WCKEY;
    } else if (!xstrcasecmp(tok, "qos")) {
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_ASSOCS;
      conf->accounting_storage_enforce |= ACCOUNTING_ENFORCE_QOS;
       ...
       */
      conf->accounting_storage_enforce &= (~ACCOUNTING_ENFORCE_NO_JOBS);
      conf->accounting_storage_enforce &= (~ACCOUNTING_ENFORCE_NO_STEPS);
      /*
      ...
      error("Invalid parameter for AccountingStorageEnforce: %s", tok);
...
static int _validate_bcast_exclude(slurm_conf_t *conf) {
  ...
  while (tok) {
    if (tok[0] != '/') {
      error("Invalid path for BcastExclude: %s", tok);
 ...
 */
static int _parse_select_type_param(char *select_type_parameters,
                                    uint16_t *param) {
  int rc = SLURM_SUCCESS;
  ...
  *param = 0;
  ...
  while (str_parameters) {
    ...
    } else if (!xstrcasecmp(str_parameters, "MULTIPLE_SHARING_GRES_PJ")) {
      *param |= SELECT_MULTIPLE_SHARING_GRES_PJ;
    } else if (!xstrcasecmp(str_parameters, "ENFORCE_BINDING_GRES")) {
      *param |= SELECT_ENFORCE_BINDING_GRES;
    } else if (!xstrcasecmp(str_parameters, "ONE_TASK_PER_SHARING_GRES")) {
    ...
    if ((*param & SELECT_CPU) && (*param & SELECT_ONE_TASK_PER_CORE)) {
      error("CR_ONE_TASK_PER_CORE is not compatible with CR_CPU*, please "
            "change to use CR_CORE* instead.");
      rc = SLURM_ERROR;
      xfree(st_str);
      return rc;
    }
...
static int _validate_and_set_defaults(slurm_conf_t *conf,
  ...
  if (!s_p_get_uint16(&conf->batch_start_timeout, "BatchStartTimeout", hashtbl))
  ...
  if (conf->cluster_name && *conf->cluster_name) {
    for (i = 0; conf->cluster_name[i] != '\0'; i++)
      conf->cluster_name[i] = (char)tolower((int)conf->cluster_name[i]);
  } else {
  ...
  if (!s_p_get_string(&conf->acct_gather_energy_type, "AcctGatherEnergyType",
                      hashtbl)) {
    /* empty */
  } else if (xstrcasestr(conf->acct_gather_energy_type, "none"))
    xfree(conf->acct_gather_energy_type);
  else
    xstrsubstituteall(conf->acct_gather_energy_type, "rsmi", "gpu");

  if (!s_p_get_string(&conf->acct_gather_profile_type, "AcctGatherProfileType",
                      hashtbl)) {
    /* empty */
  ...
  if (!s_p_get_string(&conf->acct_gather_interconnect_type,
    ...
    xstrsubstituteall(conf->acct_gather_interconnect_type, "infiniband",
                      "interconnect");
  ...
  if (!s_p_get_uint16(&conf->acct_gather_node_freq, "AcctGatherNodeFreq",
                      hashtbl))
  ...
  if (!s_p_get_string(&conf->bcast_parameters, "BcastParameters", hashtbl) &&
      s_p_get_string(&conf->bcast_parameters, "SbcastParameters", hashtbl) &&
      running_in_slurmctld())
    error("SbcastParameters has been renamed to BcastParameters. Please update "
          "your configuration.");
  ...
  (void)s_p_get_string(&conf->certgen_params, "CertgenParameters", hashtbl);
  (void)s_p_get_string(&conf->certgen_type, "CertgenType", hashtbl);

  (void)s_p_get_string(&conf->certmgr_params, "CertmgrParameters", hashtbl);
  (void)s_p_get_string(&conf->certmgr_type, "CertmgrType", hashtbl);

  (void)s_p_get_string(&conf->comm_params, "CommunicationParameters", hashtbl);
  if (running_in_slurmctld() && xstrcasestr(conf->comm_params, "NoAddrCache"))
    error("The CommunicationParameters option \"NoAddrCache\" is defunct, "
          "please remove it from slurm.conf.");
  ...
  if ((temp_str =
           xstrcasestr(conf->comm_params, "getnameinfo_cache_timeout="))) {
    slurm_conf.getnameinfo_cache_timeout = atoi(temp_str + 26);
  } else {
    slurm_conf.getnameinfo_cache_timeout = DEFAULT_GETNAMEINFO_CACHE_TIMEOUT;
  }

  conf->host_unreach_retry_count = DEFAULT_HOST_UNREACH_RETRY_COUNT;
  if ((temp_str =
           xstrcasestr(conf->comm_params, "host_unreach_retry_count="))) {
    long tmp_val = strtol(temp_str + 25, NULL, 10);
    if ((tmp_val >= 0) && (tmp_val <= INT_MAX))
      conf->host_unreach_retry_count = tmp_val;
    else
      error("CommunicationParameters option host_unreach_retry_count=%ld is "
            "invalid, ignored",
            tmp_val);
  }
  ...
  if (!s_p_get_string(&conf->cli_filter_plugins, "CliFilterPlugins", hashtbl)) {
    /* empty */
  } else if (xstrcasestr(conf->cli_filter_plugins, "none")) {
    if (xstrcasestr(conf->cli_filter_plugins, ","))
      error("Ignoring invalid CliFilterPlugins: '%s'. You can't have 'none' "
            "with another plugin.",
  ...
  if (s_p_get_string(&temp_str, "CpuFreqGovernors", hashtbl)) {
    if (cpu_freq_verify_govlist(temp_str, &conf->cpu_freq_govs)) {
      error("Ignoring invalid CpuFreqGovernors: %s", temp_str);
      conf->cpu_freq_govs =
          CPU_FREQ_ONDEMAND | CPU_FREQ_PERFORMANCE | CPU_FREQ_USERSPACE;
    }
    xfree(temp_str);
  } else {
    conf->cpu_freq_govs =
  ...
  (void)s_p_get_string(&conf->data_parser_parameters, "DataParserParameters",
                       hashtbl);
  ...
  if ((temp_str = xstrdup(getenv("SLURM_DEBUG_FLAGS"))) ||
    ...
    if (debug_str2flags(temp_str, &conf->debug_flags) != SLURM_SUCCESS) {
  ...
  (void)s_p_get_string(&conf->dependency_params, "DependencyParameters",
                       hashtbl);

  if (s_p_get_boolean(&truth, "DisableRootJobs", hashtbl) && truth)
    conf->conf_flags |= CONF_FLAG_DRJ;

  if (s_p_get_string(&temp_str, "EnforcePartLimits", hashtbl)) {
    uint16_t enforce_param;
    if (parse_part_enforce_type(temp_str, &enforce_param) != SLURM_SUCCESS) {
  ...
  if (!s_p_get_uint16(&conf->fs_dampening_factor, "FairShareDampeningFactor",
                      hashtbl))
  ...
  if (s_p_get_uint16(&uint16_tmp, "FastSchedule", hashtbl) &&
      running_in_slurmctld()) {
    if (uint16_tmp == 1)
      error("Ignoring obsolete FastSchedule=1 option. Please remove from your "
            "configuration.");
    else if (uint16_tmp == 2)
      fatal("The FastSchedule option has been removed. The FastSchedule=2 "
            "functionality is available through the "
            "SlurmdParameters=config_overrides option.");
    else
      fatal("The FastSchedule option has been removed. Please update your "
            "configuration.");
  }

  (void)s_p_get_string(&conf->fed_params, "FederationParameters", hashtbl);
  ...
  if (!s_p_get_string(&conf->interactive_step_opts, "InteractiveStepOptions",
                      hashtbl))
    conf->interactive_step_opts = xstrdup(DEFAULT_INTERACTIVE_STEP_OPTS);

  if (!s_p_get_string(&conf->job_acct_gather_freq, "JobAcctGatherFrequency",
                      hashtbl))
    conf->job_acct_gather_freq = xstrdup(DEFAULT_JOB_ACCT_GATHER_FREQ);

  if (!s_p_get_string(&conf->job_acct_gather_type, "JobAcctGatherType",
                      hashtbl)) {
    /* empty */
  } else if (xstrcasestr(conf->job_acct_gather_type, "none"))
    xfree(conf->job_acct_gather_type);

  (void)s_p_get_string(&conf->job_acct_gather_params, "JobAcctGatherParams",
                       hashtbl);
  ...
  if (conf->job_acct_gather_params) {
    ...
    if (xstrcasestr(conf->job_acct_gather_params, "UsePSS") &&
      ...
      fatal("JobAcctGatherParams options UsePSS and NoShared are mutually "
            "exclusive.");

    if (!xstrcasestr(conf->job_acct_gather_type, "linux") &&
        (xstrcasestr(conf->job_acct_gather_params, "UsePSS") ||
         xstrcasestr(conf->job_acct_gather_params, "NoShared")))
      error_in_daemon("JobAcctGatherParams UsePSS and NoShared are only "
                      "compatible with jobacct_gather/linux.");
  }
  ...
  if (!s_p_get_string(&conf->job_comp_host, "JobCompHost", hashtbl))
    conf->job_comp_host = xstrdup(DEFAULT_STORAGE_HOST);
  if (!s_p_get_string(&conf->job_comp_user, "JobCompUser", hashtbl))
  ...
  if (!s_p_get_uint32(&conf->job_comp_port, "JobCompPort", hashtbl)) {
  ...
  (void)s_p_get_string(&conf->job_submit_plugins, "JobSubmitPlugins", hashtbl);

  (void)s_p_get_uint16(&conf->health_check_interval, "HealthCheckInterval",
                       hashtbl);
  ...
  if (!s_p_get_uint32(&conf->keepalive_time, "KeepAliveTime", hashtbl)) {
    ...
    error("KeepAliveTime parameter has moved to CommunicationParameters. "
  ...
  if ((conf->keepalive_time != DEFAULT_KEEPALIVE_TIME) &&
    ...
    error("KeepAliveTime %u invalid, ignoring it.", conf->keepalive_time);
    conf->keepalive_time = DEFAULT_KEEPALIVE_TIME;
  }
  ...
  if ((temp_str = xstrcasestr(slurm_conf.comm_params, "keepaliveinterval="))) {
    long tmp_val = strtol(temp_str + 18, NULL, 10);
    if (tmp_val >= 0 && tmp_val <= INT_MAX)
      conf->keepalive_interval = tmp_val;
    else
      error("CommunicationParameters option keepaliveinterval=%ld is invalid, "
            "ignored",
            tmp_val);
  }
  conf->keepalive_probes = DEFAULT_KEEPALIVE_PROBES;
  if ((temp_str = xstrcasestr(slurm_conf.comm_params, "keepaliveprobes="))) {
    long tmp_val = strtol(temp_str + 16, NULL, 10);
    if (tmp_val >= 0 && tmp_val <= INT_MAX)
      conf->keepalive_probes = tmp_val;
    else
      error("CommunicationParameters option keepaliveprobes=%ld is invalid, "
            "ignored",
            tmp_val);
  }
  if ((temp_str = xstrcasestr(slurm_conf.comm_params, "keepalivetime="))) {
    long tmp_val = strtol(temp_str + 14, NULL, 10);
    if ((tmp_val >= 0) && (tmp_val <= INT_MAX))
      conf->keepalive_time = tmp_val;
    else
      error("CommunicationParameters option keepalivetime=%ld is invalid, "
            "ignored",
            tmp_val);
  }
  ...
  (void)s_p_get_string(&conf->launch_params, "LaunchParameters", hashtbl);

  if (xstrcasestr(conf->launch_params, "enable_nss_slurm") &&
      xstrcasestr(conf->launch_params, "disable_send_gids"))
    fatal("LaunchParameters options enable_nss_slurm and disable_send_gids are "
          "mutually exclusive.");
  ...
  if (!s_p_get_uint32(&conf->max_batch_requeue, "MaxBatchRequeue", hashtbl))
  ...
  if (!s_p_get_uint64(&conf->max_mem_per_cpu, "MaxMemPerNode", hashtbl)) {
    if (s_p_get_uint64(&conf->max_mem_per_cpu, "MaxMemPerCPU", hashtbl)) {
  ...
  if (!s_p_get_uint32(&conf->max_node_cnt, "MaxNodeCount", hashtbl))
    ...
    error("Specified MaxNodeCount in slurm.conf (%d) is higher than the "
          "maximum allowed (%d)",
  ...
  if (!s_p_get_uint32(&conf->max_step_cnt, "MaxStepCount", hashtbl))
    ...
    error("MaxStepCount=%u, No steps permitted", conf->max_step_cnt);
    return SLURM_ERROR;
  }

  if (!s_p_get_uint16(&conf->max_tasks_per_node, "MaxTasksPerNode", hashtbl)) {
    conf->max_tasks_per_node = DEFAULT_MAX_TASKS_PER_NODE;
  }

  (void)s_p_get_string(&conf->mcs_plugin_params, "MCSParameters", hashtbl);

  if (!s_p_get_string(&conf->mcs_plugin, "MCSPlugin", hashtbl)) {
    if (conf->mcs_plugin_params) {
      /* no plugin mcs and a mcs plugin param */
      error("MCSParameters=%s used and no MCSPlugin", conf->mcs_plugin_params);
      return SLURM_ERROR;
    }
  ...
  if (!conf->mcs_plugin_params && !xstrcmp(conf->mcs_plugin, "mcs/group")) {
  ...
  if (!s_p_get_string(&conf->accounting_storage_type, "AccountingStorageType",
                      hashtbl)) {
    /* empty */
    ...
    fatal("AccountingStorageType=accounting_storage/mysql only permitted in "
          "SlurmDBD.");

  (void)s_p_get_string(&conf->node_features_plugins, "NodeFeaturesPlugins",
                       hashtbl);
  ...
  if (!s_p_get_string(&conf->accounting_storage_tres, "AccountingStorageTRES",
                      hashtbl)) {
    /* Set to default accounting tres fields */
    conf->accounting_storage_tres = xstrdup(DEFAULT_ACCOUNTING_TRES);
  } else {
     ...
     * mix.
     */
    if (list_find_first(tres_list, slurm_find_char_in_list, "gres/gpu")) {
      slurm_addto_char_list(tres_list, "gres/gpumem,gres/gpuutil");
    }

    xfree(conf->accounting_storage_tres);
    conf->accounting_storage_tres = slurm_char_list_to_xstr(tres_list);
    FREE_NULL_LIST(tres_list);
  }

  if (s_p_get_string(&temp_str, "AccountingStorageEnforce", hashtbl)) {
    if (_validate_accounting_storage_enforce(temp_str, conf) != SLURM_SUCCESS) {
      error("AccountingStorageEnforce invalid: %s", temp_str);
      xfree(temp_str);
      return SLURM_ERROR;
    }
    xfree(temp_str);
  ...
  if (!s_p_get_string(&conf->accounting_storage_host, "AccountingStorageHost",
                      hashtbl))
  ...
  if (s_p_get_boolean(&truth, "AccountingStoreJobComment", hashtbl))
    fatal("The AccountingStoreJobComment option has been removed, please use "
          "AccountingStoreFlags=job_comment option instead.");

  s_p_get_string(&conf->accounting_storage_params,
                 "AccountingStorageParameters", hashtbl);

  if (!s_p_get_uint16(&conf->accounting_storage_port, "AccountingStoragePort",
                      hashtbl)) {
    if (!xstrcmp(conf->accounting_storage_type, "accounting_storage/slurmdbd"))
  ...
  (void)s_p_get_string(&conf->preempt_params, "PreemptParameters", hashtbl);
  ...
  if (s_p_get_string(&temp_str, "PriorityDecayHalfLife", hashtbl)) {
    ...
    if ((max_time < 0) && (max_time != INFINITE)) {
      error("Bad value \"%s\" for PriorityDecayHalfLife", temp_str);
      return SLURM_ERROR;
    }
  ...
  if (s_p_get_string(&temp_str, "PriorityCalcPeriod", hashtbl)) {
    ...
    if (calc_period < 1) {
      error("Bad value \"%s\" for PriorityCalcPeriod", temp_str);
      return SLURM_ERROR;
    }
  ...
  if (s_p_get_string(&temp_str, "PriorityFlags", hashtbl)) {
    ...
    if (xstrcasestr(temp_str, "NO_NORMAL_ALL"))
      conf->priority_flags |=
          PRIORITY_FLAGS_NO_NORMAL_ASSOC | PRIORITY_FLAGS_NO_NORMAL_PART |
          PRIORITY_FLAGS_NO_NORMAL_QOS | PRIORITY_FLAGS_NO_NORMAL_TRES;
  ...
  if (s_p_get_string(&temp_str, "PriorityMaxAge", hashtbl)) {
    int max_time = time_str2mins(temp_str);
    if ((max_time < 0) && (max_time != INFINITE)) {
      error("Bad value \"%s\" for PriorityMaxAge", temp_str);
      return SLURM_ERROR;
    }
  ...
  (void)s_p_get_string(&conf->priority_params, "PriorityParameters", hashtbl);
  ...
  if (s_p_get_string(&temp_str, "PriorityUsageResetPeriod", hashtbl)) {
    if (xstrcasecmp(temp_str, "none") == 0)
      ...
      error("Bad value \"%s\" for PriorityUsageResetPeriod", temp_str);
      return SLURM_ERROR;
    }
    xfree(temp_str);
  } else {
  ...
  if (!s_p_get_string(&conf->site_factor_plugin, "PrioritySiteFactorPlugin",
                      hashtbl)) {
    /* empty */
  ...
  if (!s_p_get_uint32(&conf->priority_weight_age, "PriorityWeightAge", hashtbl))
    conf->priority_weight_age = 0;
  if (!s_p_get_uint32(&conf->priority_weight_assoc, "PriorityWeightAssoc",
                      hashtbl))
    conf->priority_weight_assoc = 0;
  if (!s_p_get_uint32(&conf->priority_weight_fs, "PriorityWeightFairshare",
                      hashtbl))
    conf->priority_weight_fs = 0;
  if (!s_p_get_uint32(&conf->priority_weight_js, "PriorityWeightJobSize",
                      hashtbl))
    conf->priority_weight_js = 0;
  if (!s_p_get_uint32(&conf->priority_weight_part, "PriorityWeightPartition",
                      hashtbl))
  ...
  if (s_p_get_string(&temp_str, "PrologFlags", hashtbl)) {
    ...
    if ((conf->prolog_flags & PROLOG_FLAG_NOHOLD) &&
      ...
      fatal("PrologFlags invalid combination: NoHold cannot be combined with "
            "Contain and/or X11");
    }
    if ((conf->prolog_flags & PROLOG_FLAG_CONTAIN)) {
      /* X11 is incompatible with proctrack/linuxproc */
      if (conf->prolog_flags & PROLOG_FLAG_X11 &&
          !xstrcmp(conf->proctrack_type, "proctrack/linuxproc"))
        fatal("Invalid combination: PrologFlags=X11 cannot be combined with "
              "proctrack/linuxproc");
      /*
      ...
      if (running_in_slurmctld() &&
        ...
        error("If using PrologFlags=Contain for pam_slurm_adopt, "
              "proctrack/cgroup is required. If not using pam_slurm_adopt, "
              "please ignore error.");
    }
    if (conf->prolog_flags & PROLOG_FLAG_NOHOLD) {
  ...
  if (!s_p_get_uint16(&conf->propagate_prio_process, "PropagatePrioProcess",
                      hashtbl)) {
    conf->propagate_prio_process = PROP_PRIO_OFF;
  } else if (conf->propagate_prio_process > PROP_PRIO_NICER) {
    error("Bad PropagatePrioProcess: %u", conf->propagate_prio_process);
    return SLURM_ERROR;
  }

  if (s_p_get_string(&conf->propagate_rlimits_except,
                     "PropagateResourceLimitsExcept", hashtbl)) {
    if ((parse_rlimits(conf->propagate_rlimits_except, NO_PROPAGATE_RLIMITS)) <
        0) {
      error("Bad PropagateResourceLimitsExcept: %s",
    ...
    if (!s_p_get_string(&conf->propagate_rlimits, "PropagateResourceLimits",
                        hashtbl))
      conf->propagate_rlimits = xstrdup("ALL");
    if ((parse_rlimits(conf->propagate_rlimits, PROPAGATE_RLIMITS)) < 0) {
      error("Bad PropagateResourceLimits: %s", conf->propagate_rlimits);
      return SLURM_ERROR;
    }
  }
  ...
  if (s_p_get_string(&temp_str, "SallocDefaultCommand", hashtbl))
    fatal("SallocDefaultCommand has been removed. Please consider setting "
          "LaunchParameters=use_interactive_step instead.");

  (void)s_p_get_string(&conf->sched_params, "SchedulerParameters", hashtbl);
  ...
  if (!s_p_get_uint16(&conf->sched_time_slice, "SchedulerTimeSlice", hashtbl))
  ...
  if (s_p_get_string(&temp_str, "SelectTypeParameters", hashtbl)) {
    uint16_t type_param;
    if ((_parse_select_type_param(temp_str, &type_param) != SLURM_SUCCESS)) {
  ...
  if (!s_p_get_string(&conf->slurm_user_name, "SlurmUser", hashtbl)) {
    ...
    uid_t my_uid;
    if (uid_from_string(conf->slurm_user_name, &my_uid) != SLURM_SUCCESS) {
      error("Invalid user for SlurmUser %s, ignored", conf->slurm_user_name);
  ...
  if (!s_p_get_string(&conf->slurmd_user_name, "SlurmdUser", hashtbl)) {
    ...
    uid_t my_uid;
    if (uid_from_string(conf->slurmd_user_name, &my_uid) != SLURM_SUCCESS) {
      error("Invalid user for SlurmdUser %s, ignored", conf->slurmd_user_name);
  ...
  (void)s_p_get_string(&conf->slurmctld_addr, "SlurmctldAddr", hashtbl);
  ...
  if (!s_p_get_string(&conf->slurmctld_pidfile, "SlurmctldPidFile", hashtbl))
    conf->slurmctld_pidfile = xstrdup(DEFAULT_SLURMCTLD_PIDFILE);

  (void)s_p_get_string(&conf->slurmctld_logfile, "SlurmctldLogFile", hashtbl);
  ...
  if (s_p_get_string(&temp_str, "SlurmctldPort", hashtbl)) {
    ...
    if (end_ptr[0] == '-') {
      ...
      conf->slurmctld_port_count = port_long + 1 - conf->slurmctld_port;
    } else if (end_ptr[0] != '\0') {
      error("Invalid SlurmctldPort %s", temp_str);
      return SLURM_ERROR;
    } else {
  ...
  if (!s_p_get_uint16(&conf->slurmctld_timeout, "SlurmctldTimeout", hashtbl))
    conf->slurmctld_timeout = DEFAULT_SLURMCTLD_TIMEOUT;

  (void)s_p_get_string(&conf->slurmctld_params, "SlurmctldParameters", hashtbl);
  if (running_in_slurmctld() &&
      xstrcasestr(conf->slurmctld_params, "cloud_reg_addrs"))
    error("The SlurmctldParameters option \"cloud_reg_addrs\" is defunct, "
          "please remove it from slurm.conf.");
  ...
  if (xstrcasestr(conf->slurmd_params, "l3cache_as_socket") &&
    ...
    error_in_daemon(
        "SlurmdParameters l3cache_as_socket and numa_node_as_socket are "
        "mutually exclusive. Ignoring numa_node_as_socket.");
  ...
  (void)s_p_get_string(&conf->sched_logfile, "SlurmSchedLogFile", hashtbl);

  if (!s_p_get_uint16(&conf->sched_log_level, "SlurmSchedLogLevel", hashtbl))
  ...
  if (!s_p_get_string(&conf->state_save_location, "StateSaveLocation", hashtbl))
    conf->state_save_location = xstrdup(DEFAULT_SAVE_STATE_LOC);

  (void)s_p_get_string(&conf->suspend_exc_nodes, "SuspendExcNodes", hashtbl);
  (void)s_p_get_string(&conf->suspend_exc_parts, "SuspendExcParts", hashtbl);
  (void)s_p_get_string(&conf->suspend_exc_states, "SuspendExcStates", hashtbl);
  (void)s_p_get_string(&conf->suspend_program, "SuspendProgram", hashtbl);
  ...
  if (s_p_get_string(&temp_str, "SuspendTime", hashtbl)) {
    if (!xstrcasecmp(temp_str, "NONE") || !xstrcasecmp(temp_str, "INFINITE") ||
  ...
  (void)s_p_get_string(&conf->switch_param, "SwitchParameters", hashtbl);
  ...
  if (s_p_get_string(&temp_str, "TaskPluginParam", hashtbl)) {
    ...
    while (tok) {
      if (xstrcasecmp(tok, "none") == 0) {
      ...
      } else if (xstrncasecmp(tok, "autobind=", strlen("autobind=")) == 0) {
        ...
        if (xstrcasecmp(val_ptr, "none") == 0) {
          set_auto = true;
        } else if (xstrcasecmp(val_ptr, "threads") == 0) {
          set_auto = true;
          conf->task_plugin_param |= CPU_AUTO_BIND_TO_THREADS;
        } else if (xstrcasecmp(val_ptr, "cores") == 0) {
          set_auto = true;
          conf->task_plugin_param |= CPU_AUTO_BIND_TO_CORES;
        } else if (xstrcasecmp(val_ptr, "sockets") == 0) {
          set_auto = true;
          conf->task_plugin_param |= CPU_AUTO_BIND_TO_SOCKETS;
        } else {
        ...
        if (!xstrstr(conf->task_plugin, "task/cgroup")) {
          error("TaskPluginParam=SlurmdSpecOverride must be used with "
                "task/cgroup");
          return SLURM_ERROR;
        }
        if (!xstrstr(conf->select_type, "cons_tres")) {
          error(
              "TaskPluginParam=SlurmdSpecOverride must be used with cons/tres");
          return SLURM_ERROR;
        }
  ...
  if ((conf->task_plugin_param & SLURMD_OFF_SPEC) &&
      (conf->task_plugin_param & SLURMD_SPEC_OVERRIDE)) {
    error("TaskPluginParams SlurmdOffSpec and SlurmdSpecOverride are mutually "
          "exclusive");
    return SLURM_ERROR;
  }
  ...
  if (conf->topology_param) {
    ...
    if (xstrcasestr(conf->topology_param, legacy_var) &&
      ...
      xstrfmtcat(conf->comm_params, "%s%s", conf->comm_params ? "," : "",
                 legacy_var);
  ...
  if (!xstrcasecmp(conf->select_type, "select/linear") &&
    ...
    fatal("select/linear with topology/tree is not supported. Please switch to "
          "select/cons_tres or stop using topology/tree.");

  if (((conf->tree_width =
            (getenv("SLURM_TREE_WIDTH") ? atoi(getenv("SLURM_TREE_WIDTH"))
                                        : 0)) > 0) ||
  ...
  s_p_get_string(&conf->unkillable_program, "UnkillableStepProgram", hashtbl);
  default_unkillable_timeout = conf->msg_timeout * 5;
  if (!s_p_get_uint16(&conf->unkillable_timeout, "UnkillableStepTimeout",
                      hashtbl)) {
    if (default_unkillable_timeout > UINT16_MAX) {
      error_in_daemon("The default value for UnkillableStepTimeout of "
                      "MessageTimeout*5 (%u) is larger than the maximum "
                      "allowed value for UnkillableStepTimeout (%u), setting "
                      "UnkillableStepTimeout to the maximum value.",
                      default_unkillable_timeout, UINT16_MAX);
      conf->unkillable_timeout = UINT16_MAX;
    } else
      conf->unkillable_timeout =
          MAX(DEFAULT_UNKILLABLE_TIMEOUT, default_unkillable_timeout);
  } else if (conf->unkillable_timeout < default_unkillable_timeout)
    error_in_daemon("UnkillableStepTimeout must be at least 5 times greater "
                    "than MessageTimeout, otherwise nodes may go down with the "
                    "reason \"KillTaskFailed\". Current values: "
                    "UnkillableStepTimeout=%u, MessageTimeout=%u",
  ...
  (void)s_p_get_string(&conf->requeue_exit_hold, "RequeueExitHold", hashtbl);
...
extern uint16_t prolog_str2flags(char *prolog_flags) {
  ...
  while (tok) {
    if (xstrcasecmp(tok, "Alloc") == 0)
      ...
      rc |= (PROLOG_FLAG_ALLOC | PROLOG_FLAG_CONTAIN | PROLOG_FLAG_RUN_IN_JOB);
      ...
      rc |= (PROLOG_FLAG_ALLOC | PROLOG_FLAG_FORCE_REQUEUE_ON_FAIL);
    else if (xstrcasecmp(tok, "Serial") == 0)
      rc |= PROLOG_FLAG_SERIAL;
    else if (xstrcasecmp(tok, "X11") == 0) {
#ifdef WITH_SLURM_X11
      rc |= (PROLOG_FLAG_ALLOC | PROLOG_FLAG_CONTAIN | PROLOG_FLAG_X11);
#else
...
extern int debug_str2flags(const char *debug_flags, uint64_t *flags_out) {
  ...
  while (tok) {
    ...
    else if (!xstrcasecmp(tok, "JobAccountGather") || !xstrcasecmp(tok, "JAG"))
      (*flags_out) |= DEBUG_FLAG_JAG;
    else if (!xstrcasecmp(tok, "JobComp") || !xstrcasecmp(tok, "Elasticsearch"))
    ...
    else if (xstrcasecmp(tok, "Network") == 0 || xstrcasecmp(tok, "Net") == 0)
    ...
    else if (!xstrcasecmp(tok, "WorkQueue") || !xstrcasecmp(tok, "WorkQ") ||
...
extern int unpack_config_plugin_params(void **object, uint16_t protocol_version,
  ...
  *object = object_ptr;
  ...
  if (unpack_key_pair_list((void *)&object_ptr->key_pairs, protocol_version,
                           buff) != SLURM_SUCCESS)
    goto unpack_error;
...
extern int unpack_config_plugin_params_list(void **plugin_params_l,
  ...
  if (count != NO_VAL) {
    ...
    int i;
    for (i = 0; i < count; i++) {
      if (unpack_config_plugin_params((void *)&object, protocol_version,
                                      buff) == SLURM_ERROR)
        goto unpack_error;
...
extern void pack_key_pair_list(void *key_pairs, uint16_t protocol_version,
  ...
  if (count && (count != NO_VAL)) {
    ...
    while ((key_pair = list_next(itr))) {
      pack_config_key_pair(key_pair, protocol_version, buffer);
    }
    list_iterator_destroy(itr);
  }
}
...
extern int unpack_key_pair_list(void **key_pairs, uint16_t protocol_version,
  ...
  if (count != NO_VAL) {
    ...
    int i;
    for (i = 0; i < count; i++) {
      if (unpack_config_key_pair((void *)&object, protocol_version, buffer) ==
          SLURM_ERROR)
        goto unpack_error;
      list_append(tmp_list, object);
    }
...
extern char *get_extra_conf_path(char *conf_name) {
  ...
  if (config_files &&
      (cached_config =
           list_find_first(config_files, find_conf_by_name, conf_name)) &&
...
extern int add_remote_nodes_to_conf_tbls(char *node_list,
  ...
  if ((host_list = hostlist_create(node_list)) == NULL) {
    error("hostlist_create error for %s: %m", node_list);
    return SLURM_ERROR;
  }
  ...
  while ((hostname = hostlist_shift(host_list))) {
    ...
    _push_to_hashtbls(hostname, hostname, NULL, NULL, 0, &node_addrs[i++], true,
                      true, false);
    free(hostname);
  }
  slurm_conf_unlock();
  ...
  while (p_curr) {
    if (!xstrcmp(p_curr->alias, node_name)) {
      if (p_prev)
        ...
        node_to_host_hashtbl[alias_idx] = p_curr->next_alias;
      break;
    }
